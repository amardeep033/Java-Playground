...
| Open Session in View (OSIV) | Keeps the Hibernate persistence context/session open through the whole web request. |
...
| OSIV | Avoid relying on Open Session in View to hide lazy-loading problems; fetch needed data inside the service transaction. |
...
| What is Open Session in View (OSIV)? | A Spring/Hibernate pattern that keeps the persistence context/session open through the web request, even after service-layer work returns. |
| What is the risk with OSIV? | Controllers or JSON serialization can trigger lazy SQL queries outside the service transaction, causing hidden N+1 queries and unclear database boundaries. |
